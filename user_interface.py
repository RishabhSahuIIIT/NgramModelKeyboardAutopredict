...
class TerminalUI:
    ...
    def calculate_scores(self, text: str) -> List[int]:
        """Calculate scores for the terminal UI."""
        # Total letters typed
        total_letters = len([char for char in text if char.isalpha()])
        
        # Total tab presses (tracked elsewhere in the UI)
        # We'll use an instance variable to track this
        if not hasattr(self, '_total_tab_presses'):
            self._total_tab_presses = 0
        
        # Average letters per word
        words = text.split()
        avg_letters_per_word = round(total_letters / len(words), 2) if words else 0
        
        # Average tab presses per word
        avg_tab_presses_per_word = round(self._total_tab_presses / len(words), 2) if words else 0
        
        return [
            total_letters, 
            self._total_tab_presses, 
            avg_letters_per_word, 
            avg_tab_presses_per_word
        ]
    ...
    def draw_scores_panel(self) -> None:
        ...
        #  Set score labels
        score_labels = ["Total letters Typed","Tab presses","Average no of letters typed","Average no of Tab presses"]
    ...
    def handle_input(self, key) -> bool:
        ...
        if key == 10:  # Enter key
            if self.suggestions and self.current_suggestion_idx < len(self.suggestions):
                ...
                self.suggestions = []
                self.current_suggestion_idx = 0
                self._total_tab_presses+=self.tabPressForWord
                self.tabPressForWord=0
            return True
    ...
    if sys.argv[1].endswith(".txt"):
        ...
        for corpus_file_path in corpus_file_path_list:
            try:
                ...
                sys.exit(1)
            corpus += cur_corpus
    inputPara="Friquet had put on his Sunday clothes, under the pretext of having a swollen face which he had simulated by introducing a handful of cherry kernels into one side of his mouth, and had procured a whole holiday from Bazin. On leaving Bazin, Friquet started for the Palais Royal, where he arrived at the moment of the turning out of the regiment of guards; and as he had only gone there for the enjoyment of seeing it and hearing the music, he took his place at their head, beating the drum on two pieces of slate and passing from that exercise to that of the trumpet, which he counterfeited prtty naturally with his mouth in a manner which had more than once called forth the praises of amateurs of imitative harmony. "
    # Excerpt from "A Study in Scarlet" (sherlock holmes ) by Arthur Conan Doyle
    inputPara2="“I hardly expected that you would. Let me see if I can make it clearer. Most people, if you describe a train of events to them, will tell you what the result would be. They can put those events together in their minds, and argue from them that something will come to pass. There are few people, however, who, if you told them a result, would be able to evolve from their own inner consciousness what the steps were which led up to that result. This power is what I mean when I talk of reasoning backwards, or analytically.”“I understand,” said I."
    n = 3
    model = NgramCharacterModel(corpus, n)
    
    ui = TerminalUI(model,inputPara2)
    ui.run()
